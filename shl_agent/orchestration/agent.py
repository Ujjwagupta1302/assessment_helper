...
from shl_agent.vector_store import embed_and_search, collection_info
...
def _build_filtered_catalog(conversation_text: str) -> str:
    """
    Use vector search to find the top-K relevant catalog items for the
    current conversation. Falls back to the full catalog if vector store
    is unavailable.
    """
    if not _VECTOR_STORE_READY:
        return get_catalog_for_prompt()

    urls = embed_and_search(conversation_text, top_k=TOP_K_CANDIDATES)
    if not urls:
    ...
    logger.info(
        "Filtered catalog: %d candidates selected via vector search "
        "(prompt size: %d chars / ~%d tokens)",
        len(urls), len(filtered_json), len(filtered_json) // 4,
    )
